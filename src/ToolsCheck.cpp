...
static std::string write_temp_script(const std::string &content) {
    char tmpl[] = "/tmp/host_tools_check_XXXXXX.sh"; // mkstemp replaces XXXXXX
    int fd = mkstemps(tmpl, 3); // keep .sh suffix (3 chars)
    if (fd == -1) {
        return {};
    }
    ssize_t remaining = (ssize_t)content.size();
    const char* buf = content.c_str();
    while (remaining > 0) {
        ssize_t written = write(fd, buf, (size_t)remaining);
        if (written <= 0) {
            close(fd);
            unlink(tmpl);
            return {};
...
bool run_host_tools_check() {
    static const char* script = R"BASH(#!/bin/bash
...
if [ "$(nproc)" = "" ]; then
   ...
   echo "OK: nproc reports $(nproc) logical cores are available"
    ...
    std::string path = write_temp_script(script);
    if (path.empty()) {
        std::fprintf(stderr, "Failed to create temporary script file.\n");
        return false;
    }

    // Execute the script with /bin/bash. Output will go directly to this process's stdout/stderr.
    std::string cmd = "/bin/bash \"" + path + "\"";
